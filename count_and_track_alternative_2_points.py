...
#video = cv2.VideoWriter('Tracking_Counting_With_Time_2Points.mp4', fourcc, 20, (int(width), int(height)))
...
for frame in frames_generator:
    ...
    #img = cv2.circle(orig_img, ENTRANCEPOINT, 1, color=(255, 0, 0), thickness=2)
    #img = cv2.circle(img, EXITPOINT, 1, color=(255, 0, 0), thickness=2)
    ...
    if tracker.lost_tracks:
        for losttrack in tracker.lost_tracks:
            if losttrack not in lost_tracks_list:
                ...
                last_x_position = track_history[losttrack.external_track_id][-1][0]
                last_position = [last_x_position, last_y_position]
                first_y_position = track_history[losttrack.external_track_id][0][1]

                #se entrada mais perto E first_y > last_y = entrada
                if ((math.dist(ENTRANCEPOINT, last_position)) < (math.dist(EXITPOINT, last_position)) and ((first_y_position - last_y_position) > 0 or (first_y_position == last_y_position))):
                    alt_in_count = alt_in_count + 1
                    print('FIRST AND LAST', first_y_position, last_y_position)
                    #input("check")
                
                #se saída mais perto E first_y < last_y = saída
                if ((math.dist(ENTRANCEPOINT, last_position)) > (math.dist(EXITPOINT, last_position)) and (first_y_position - last_y_position) < 0):
                    alt_out_count = alt_out_count + 1
                    print('FIRST AND LAST', first_y_position, last_y_position)
                    #input("check")#se entrada mais perto E last_y > first_y = saída (essa movimentação conta mais), mas tenho q comparar ela com proximidade
                
                #se entrada mais perto E first_y < last_y = entrada (entrada contradizendo Y)
                elif ((math.dist(ENTRANCEPOINT, last_position)) < (math.dist(EXITPOINT, last_position)) and (first_y_position - last_y_position) < 0): 
                    alt_in_count = alt_in_count + 1
                    #unsure_count = unsure_count + 1
                
                #se saída mais perto E last_y > first_y = saída com certeza
                elif ((math.dist(ENTRANCEPOINT, last_position)) > (math.dist(EXITPOINT, last_position)) and (first_y_position - last_y_position) > 0): 
                    unsure_count = unsure_count + 1
                
                #se saída mais perto E last < first -> não tenho ctz, mas to contando como out
                #else:
                #    alt_out_count = alt_out_count + 1
                '''
                first_y_position = track_history[losttrack.track_id][0][1]
                last_y_position = track_history[losttrack.track_id][-1][1]
    ...
    #annotator._annotate_anything_count(annotated_frame, sv.Point(150, 270), f"UNSURE {unsure_count}")
